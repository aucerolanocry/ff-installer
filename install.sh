...
# ===== BARRA DE PROGRESSO VERMELHA PISCANTE (PARA SINCRONIZAÇÃO) =====
progress_bar() {
    local current=$1
    local total=$2
...
# ===== BARRA DE PROGRESSO MAIS RÁPIDA E FLUIDA PARA DOWNLOAD =====
progress_bar_download() {
    local pid=$1
    local arquivo=$2
    local total_size=$(stat -c %s "$arquivo" 2>/dev/null || echo "1000000")
    local last_progress=0
    
    while kill -0 $pid 2>/dev/null; do
        if [ -f "$arquivo" ]; then
            local current_size=$(stat -c %s "$arquivo" 2>/dev/null || echo "0")
            local percent=$((current_size * 100 / total_size))
            
            # Só atualiza se mudou (evita processamento desnecessário)
            if [ $percent -ne $last_progress ]; then
                last_progress=$percent
                local filled=$((percent / 2))
                local empty=$((50 - filled))
                
                # Efeito piscante mais rápido (alterna a cada 2% em vez de 1%)
                if ((percent % 4 < 2)); then
                    color="\033[1;31m"
                else
                    color="\033[0;31m"
        ...
        sleep 0.05  # Reduzido de 0.1 para 0.05 (mais rápido)
    done
    printf "\r\033[1;31m[██████████████████████████████████████████████████] 100%%\033[0m\n"
}

# ===== DOWNLOAD MAIS RÁPIDO COM EXCLUSÃO =====
download_obb() {
    local tipo=$1
    local url=$2
    local arquivo=$3
    local destino=$4
    local arquivo_destino="${destino}${arquivo}"
    
    echo -e "$red [↓] Baixando OBB do $tipo...$reset"
    echo ""
    
    # Verificar se já existe OBB no destino e excluir
    if [ -f "$arquivo_destino" ]; then
        echo -e "${silver}[•] OBB antiga encontrada. Removendo...$reset"
        rm -f "$arquivo_destino"
        echo -e "${white}[⛥] OBB antiga removida.$reset"
        echo ""
    fi
    
    # Usar curl com buffer maior e conexão keep-alive para download mais rápido
    curl -L --speed-time 30 --speed-limit 1024 -k -o "$arquivo" "$url" &
    local pid=$!
    
    # Barra de progresso mais rápida
    progress_bar_download $pid "$arquivo"
    wait $pid
    ...
    if [ -f "$arquivo" ] && [ -s "$arquivo" ]; then
        ...
        echo -e "${white}[⛥] OBB instalada com sucesso em: $destino$reset"
        return 0
    else
        echo ""
        echo -e "$red [☠︎] Erro no download!$reset"
        return 1
    fi
}
...
sincronizar_dados() {
    ...
    echo -e "${silver}[•] Verificando pasta do jogo: $origem${reset}"
    ...
    # 3. Sincronizar TODAS as subpastas
    ...
    if [ -f "$temp_file" ] && [ -s "$temp_file" ]; then
        ...
        echo -e "${white}    ✓ $subpastas subpastas sincronizadas${reset}"
        rm -f "$temp_file"
    else
    ...
    # 6. Sincronizar arquivos em MReplays (com caminho corrigido)
    ...
    if [ "$tipo" = "normal" ]; then
        ...
        mreplays_path="/storage/emulated/0/Android/data/com.dts.freefiremax/files/MReplays/"
    fi
    ...
    if [ -f "$temp_file" ] && [ -s "$temp_file" ]; then
        ...
        echo -e "${white}    ✓ $arquivos_mreplays arquivos em MReplays sincronizados${reset}"
        rm -f "$temp_file"
    else
        echo -e "${silver}    → Nenhum arquivo em MReplays encontrado${reset}"
    fi
    ...
    echo -e "${white}│  ${silver}📂  Todas as pastas sincronizadas                 ${white}│${reset}"
    ...
    echo -e "${white}│  ${silver}📄  Arquivos MReplays sincronizados                ${white}│${reset}"
    echo -e "${white}└─────────────────────────────────────────────────────┘${reset}"
...
while true; do
    ...
    if [ "$menu" = "1" ]; then
        ...
        while true; do
            ...
            if [ "$obb" = "1" ]; then
                download_obb "FF NORMAL" \
                    "https://github.com/aucerolanocry/ff-installer/releases/download/v1/main.2019116013.com.dts.freefireth.obb" \
                    "main.2019116013.com.dts.freefireth.obb" \
                    "/storage/emulated/0/MIUI/sound_recorder/fm_rec/"
                ...
                download_obb "FF MAX" \
                    "https://github.com/aucerolanocry/ff-installer/releases/download/v2/main.2019116013.com.dts.freefiremax.obb" \
                    "main.2019116013.com.dts.freefiremax.obb" \
                    "/storage/emulated/0/MIUI/sound_recorder/fm_rec/"
